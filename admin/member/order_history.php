...
<head>
    ...
    <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
    <style>
        :root { --mira-pink: #b3365b; --mira-soft-pink: #fdf2f4; --mira-text: #4a4a4a; }
        body { background-color: var(--mira-soft-pink); font-family: 'Sarabun', sans-serif; color: var(--mira-text); }
        .mira-card { background: white; border-radius: 1.5rem; border: none; box-shadow: 0 10px 30px rgba(179, 54, 91, 0.05); padding: 25px; }
        .btn-mira { background: var(--mira-pink); color: white; border-radius: 50px; padding: 8px 25px; border: none; transition: 0.3s; }
        .btn-mira:hover { background: #8e2a48; color: white; transform: translateY(-2px); }
        
        /* Status Badges */
        .badge { border-radius: 50px; padding: 6px 12px; font-weight: 500; font-size: 0.75rem; }
        ...
        .order-table thead th { background: #f9fafb; color: #94a3b8; font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.5px; border: none; padding: 15px; }
        .order-table tbody td { vertical-align: middle; padding: 15px; border-bottom: 1px solid #f1f5f9; font-size: 0.9rem; }
        .product-info { font-size: 0.85rem; color: #64748b; line-height: 1.4; }
        .search-box { border-radius: 50px; border: 1px solid #e2e8f0; padding-left: 20px; }
        .back-link {
    transition: 0.3s;
    color: #8e9aaf !important; /* สีเทาอ่อนๆ ตามรูป */
}
.back-link:hover {
    color: var(--mira-pink) !important;
    transform: translateX(-5px); /* ขยับซ้ายนิดนึงเวลา hover */
}
    </style>
</head>
<body>
...
<div class="modal fade" id="returnModal" tabindex="-1" aria-hidden="true">
            ...
            <table class="table order-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <small class="d-block"><?= $o['payment_method'] ?? 'N/A' ?></small>
                            <span class="text-xs <?= $o['payment_status'] == 'success' ? 'text-success' : 'text-warning' ?>">
                                ● <?= $o['payment_status'] == 'success' ? 'ชำระแล้ว' : 'รอตรวจสอบ' ?>
                            </span>
                        </td>
    ...
    <?php if ($o['status'] == 'completed' || $o['status'] == 'shipped'): ?>
    <li>
        <a class="dropdown-item small text-warning" href="javascript:void(0)" onclick="openReturnModal(<?= $o['order_id'] ?>)">
            <i class="bi bi-arrow-counterclockwise me-2"></i>คืนสินค้า
        </a>
    </li>
    <?php endif; ?>
    ...
    <li>
        <a class="dropdown-item small text-danger" href="update_order_status.php?order_id=<?= $o['order_id'] ?>&new_status=cancelled" onclick="return confirm('ยืนยันการยกเลิกออเดอร์นี้?')">
            <i class="bi bi-x-circle me-2"></i>ยกเลิกออเดอร์
        </a>
    </li>
    <?php endif; ?>
</ul>
